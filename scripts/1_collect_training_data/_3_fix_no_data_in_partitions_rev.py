...
def read_raster_to_np_and_write_to_tif(input_raster,output_dir,topo_nodata= -9999,optical_nodata= -32768,class_nodata=None,fill_value=0,topo_band=-3,optical_band=0,class_band=9,optical_start=0,topo_start=5,**kwargs): #note that it will be a numpy array which is zero indexed 
	...
	output_filename = os.path.join(output_dir,f'{os.path.split(input_raster)[1][:-4]}_noData_corrected.tif')
	...
	if not os.path.exists(output_filename): 
		...
		with rasterio.open(input_raster) as src: #window works like Window(col_off, row_off, width, height)
			...
			if kwargs.get('alt_topo_band'): 
				...
				#checks the logical value of multiple boolean masks (e.g. True, True, False should be False). This is done on an element-wise basis 
...
def main(vrt_dir,**kwargs): 
		...
		#use this file to test partitions
		...
		#use this file to test full study area vrts
		#test_file = '/vol/v3/ben_ak/raster_files/neural_net/glacier_cnn_full_study_area_vrts/southern_region/full_time_series_year_2002_revised_class_label_multiband.vrt'
		# test_file = "/vol/v3/ben_ak/raster_files/neural_net/tifs/revised_time_series/subset_12/full_time_series_year_2010_revised_class_label_multiband_clipped.tif"
		# output_dir = "/vol/v3/ben_ak/raster_files/neural_net/testing/noData/"
		# read_raster_to_np_and_write_to_tif(test_file,output_dir,alt_topo_band=-4)
		###########################################################
		#working
		time0 = time.time()
		count = 0 
		# for subset in range(11,17):
		# 	if count == 0:  
		# 		file_dir = vrt_dir.replace('10',f'{subset}')
		# 	else: 
		# 		file_dir = vrt_dir.replace('10', f'{subset+1}')
		# 	print(file_dir)
		# 	count +=1
		file_list = glob.glob(os.path.join(vrt_dir,'*.tif'))
