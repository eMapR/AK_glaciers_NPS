...
class model_trainer():
    ...
    def __init__(self, model, model_form_id, training_dir, dev_dir, stats_dir, 
                 weights_dir, model_weights=None, batch_size=6, max_epochs=200, 
    ...
    def __get_data_loaders(self):
        ...
        training_dataset = GlacierDataset(self.training_dir, transform=data_transforms)
#        dev_dataset = GlacierDataset(self.dev_dir, transform=None)
        
        # Define the dataset loaders
        train_loader = torch.utils.data.DataLoader(training_dataset, batch_size=self.batch_size, shuffle=True, num_workers=0, pin_memory=True)
...
#        train_loader, dev_loader, test_loader = self.__get_data_loaders()
        train_loader = self.__get_data_loaders()
        ...
        if self.model_weights is not None:
            self.configured_network.load_state_dict(torch.load(self.model_weights)).to(torch.device(self.device))
        ...
        for epoch in range(self.max_epochs):
            ...
            for i, data in enumerate(Bar(train_loader)):
                ...
                iteration_counter += 1
                self.training_loss.append([iteration_counter, losses.jaccard_score(outputs, labels).item(), self.__compute_gradient_norm()])
...
if __name__ == "__main__":
    ...
    training_dir = "/mnt/d/glacier_tensors/training_data"
    dev_dir = "/mnt/d/glacier_tensors/validation_data"
    
    # Define the directories two write out the S
    write_stats_dir = "/mnt/e/glacier_data/model_run_tables"
    write_weights_dir = "/mnt/e/glacier_data/model_weights"
...
#        CRF(n_spatial_dims=2)
    ...
    trainer = model_trainer(model = model, 
                            ...
                            drop_lr_n_epochs = 2,
                            print_error = True,
                            log_error_every_n_epochs = 50,
                            device = 'cuda:0'
                            )
    
    # Train the model   
    trainer.train_model(learning_rate=0.0001)   
