...
class ProcessBigImages():
    ...
    def __init__(self, image_path, output_dir, output_name, chunk_size):
        ...
        self.n_chunks_x = ceil(source.width / float(self.chunk_size)) * 2
        self.n_chunks_y = ceil(source.height / float(self.chunk_size)) * 2
    ...
    def __generate_prediction_tiles(self, network):
        ...
        for y in tqdm(range(0, int(self.n_chunks_y))):  
            ...
            for x in range(0, int(self.n_chunks_x)):  
                ...
                x_offset = (x * self.chunk_size) / 2
                y_offset = (y * self.chunk_size) / 2
                ...
                # Clip out the center of the chip
                start_index = self.chunk_size/4
                end_index = self.chunk_size - start_index
                prediction = prediction[start_index:start_index, end_index:end_index]
...
#        CRF(n_spatial_dims=2)
    ...
    folders_containing_ts = glob(time_series_dir+"\\*")[0:1]
